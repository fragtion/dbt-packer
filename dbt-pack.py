...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='dbt-pack.py', description='.dbp file packer/unpacker for Diabotical')
    ...
    parser_unpack = subparsers.add_parser('unpack', aliases=['u'], help='unpack the .dbp into the current directory')
    parser_unpack.add_argument('source', type=argparse.FileType('r'), help="source file")

    parser_pack = subparsers.add_parser('pack', aliases=['p'], help='pack a folder into a .dbp file')
    parser_pack.add_argument('source', type=dir_path, help="source folder")
    ...
    if args.command.startswith("l"):
        ...
        for df in d.index:
            path = Path(PureWindowsPath(df.name))
            print(path)
            os.makedirs(path.parents[0], 0o766, True)
            io.open(path, "wb").write(d.read_file(df))
        print("DONE")
